  ...
  return (
    <section className="py-16 sm:py-24 bg-[#f6f6fb]">
      <div className="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8">
        <h2 className="text-2xl sm:text-3xl font-bold" style={{ color: COLORS.navy }}>Why GoSource</h2>
        <div className="mt-8 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {items.map(({ title, icon: Icon, desc }) => (
            <div key={title} className="group relative rounded-xl border bg-white p-6 shadow-sm transition-all hover:shadow-md" style={{ borderColor: COLORS.gray }}>
              <div className="mb-4 inline-flex h-10 w-10 items-center justify-center rounded-lg" style={{ background: COLORS.yellow }}>
                <Icon className="h-5 w-5 text-black" />
              </div>
              <h3 className="text-lg font-semibold" style={{ color: COLORS.navy }}>{title}</h3>
              <p className="mt-2 text-sm text-[#00003d]/70">{desc}</p>
            </div>
          ))}
        </div>
      </div>
